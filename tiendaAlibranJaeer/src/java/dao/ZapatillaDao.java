...
public class ZapatillaDao extends DAO {
    ...
    public ArrayList<Zapatilla> listaZapatilla() throws Exception {
        ...
        try {
            this.conectar();
            query = "SELECT pro.id_producto,pro.nombre_producto,pro.precio_costo,pro.precio_venta,pro.margen_ganancia,\n"
                    + "pro.descripcion,pro.cantidad,su.nombre_sucursal,talla.nombre_zapatilla,tipo.nombre_tipo_zapatilla\n"
                    + "from productoinventario as pro INNER JOIN sucursal as su\n"
                    + "on pro.id_sucursal = su.id_sucursal INNER JOIN talla_zapatilla as talla\n"
                    + "on pro.id_talla_zapatilla = talla.id_talla_zapatilla INNER JOIN tipo_zapatilla as tipo\n"
                    + "on pro.id_tipo_zapatilla = tipo.id_tipo_zapatilla";
            ...
            while (res.next()) {
                ...
                za.setTalla(res.getString("nombre_zapatilla"));
                za.setTipo(res.getString("nombre_tipo_zapatilla"));
                za.setSucursal(res.getString("nombre_sucursal"));
    ...
    public void modificar(Zapatilla za) throws Exception {
        try {
            ...
            sta = this.getCn().prepareStatement(query);
            sta.setString(1, za.getNombre_producto());
            sta.setDouble(2, za.getPrecio_costo());
            sta.setDouble(3, za.getPrecio_venta());
            sta.setDouble(4, za.getMargen_ganancia());
            sta.setString(5, za.getDescripcion());
            sta.setInt(6, za.getCantidad());
            sta.setInt(7, za.getId_producto());
            sta.executeUpdate();
